...
const ConsultaGeneralPDFDoc = ({ data, logoSrc }) => {
  const {
    id_paciente,
    num_expediente,
    nombre_completo,
  ...
  return (
    <Document author="Sistema de Registro" title={`Consulta General ${nombre_completo || "-"}`}>
          ...
          <View style={styles.header}>
            <View style={styles.logoWrap}>
              ...
              <View style={styles.titleBox}>
                <Text style={styles.title}>Ficha de Registro — Consulta General</Text>
                <Text style={styles.subtitle}>Centro de Salud | Registro de pacientes</Text>
              </View>
            </View>
...
export const generarPDF = async (id) => {
  try {
    const res = await fetch(`${API_URL}consulta/general/${id}`);
    ...
    const blob = await pdf(<ConsultaGeneralPDFDoc data={data} logoSrc={logoSrc} />).toBlob();

    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `consultageneral_${id}.pdf`;
...
export const ConsultaGeneralPDF = ({ data, logoSrc }) => <ConsultaGeneralPDFDoc data={data} logoSrc={logoSrc} />;
export default ConsultaGeneralPDF;

...
const PrenatalPDFDoc = ({ data, logoSrc }) => {
  const {
    id_paciente,
    num_expediente,
    nombre_completo,
  ...
  return (
    <Document author="Sistema de Registro" title={`Prenatal ${nombre_completo || "-"}`}>
        ...
        <View style={styles.card}>
          <View style={styles.header}>
            <View style={styles.logoWrap}>
              ...
              <View style={styles.titleBox}>
                <Text style={styles.title}>Ficha de Registro — Prenatal</Text>
                <Text style={styles.subtitle}>Centro de Salud | Registro de mujeres embarazadas</Text>
              </View>
            </View>
          ...
          <View style={styles.grid}>
            ...
            <View style={styles.field}>
              ...
              <Text style={styles.value}>{fmtDate(primer_control_post_parto)}</Text>
            </View>
            <View style={styles.field}>
              <Text style={styles.label}>Fecha Segundo control post parto</Text>
              <Text style={styles.value}>{fmtDate(segundo_control_post_parto)}</Text>
            </View>
            <View style={styles.field}>
...
export const generarPDF = async (id) => {
  try {
    const res = await fetch(`${API_URL}prenatal/${id}`);
    ...
    const blob = await pdf(<PrenatalPDFDoc data={data} logoSrc={logoSrc} />).toBlob();

    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `prenatal_${id}.pdf`;
...
export const PrenatalPDF = ({ data, logoSrc }) => <PrenatalPDFDoc data={data} logoSrc={logoSrc} />;
export default PrenatalPDF;
